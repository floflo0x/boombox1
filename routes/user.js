...
router.get("/", async (req, res, next) => {
  try {
    ...
    const response8 = await axios.request(getData("news/get/trending.php", "post", postData8));
...
router.get("/filter/:id", async (req, res, next) => {
  ...
  try {
    ...
    const response6 = await axios.request(
      getData("get/categories_post.php", "post", postData5)
    );
...
router.get("/news/:id/:nt", async (req, res, next) => {
  try {
    ...
    const response6 = await axios.request(
      getData("get/categories_post.php", "post", postData3)
    );
...
router.get("/poll/:id1/:id2/:title", async (req, res, next) => {
  try {
    ...
    if (id1 != 0 && id2 != 0 && modalShown) {
      ...
      // console.log(data2, data3, data2[0].question.landscape_img);
...
router.post("/pay", 
	[
		body('crypto')
	  	...
	  	.isIn(['ETH', 'DGB', 'GAS', 'LTC', 'XLM', 'XRP', 'DASH', 'TRX', 'EOS', 'XMR', 
             'BCH', 'ZEN', 'XVG', 'TUSD', 'QTUM', 'FUN', 'BTG', 'BCD', 'BAT', 'USDTTRC20'])
	...
	async (req, res, next) => {
    try {
      const { uId, rId, size, gender, crypto } = req.body;
      // const curry = req.body.crypto;

      // console.log(uId, rd, size, gender, crypto);
      ...
      if (!error.isEmpty()) {
        ...
        let data = JSON.stringify({
          ...
          "price_currency": "usd",
          "pay_currency": crypto,
          "ipn_callback_url": "https://api.hiphopboombox.com/cryptoAPI/b.php",
          "order_id": "RGDBP-21314",
          "order_description": "Sneakers with size " + size
        });
        ...
        let z = response.data;
          ...
          return res.redirect(`/qr?id=${z?.payment_id}&uId=${uId}&rId=${rId}&s=${size}&g=${gender}`);
        }
      }
    }
